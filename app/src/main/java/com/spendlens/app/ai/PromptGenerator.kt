...
object PromptGenerator {
    ...
    fun generate(smsList: List<RawSmsEntity>): String {
        return buildString {
            ...
            append("Next, decide isReminder: true if the SMS describes money that WILL be debited/credited in the ")
            append("future rather than something that already happened — e.g. \"Rs 129 will be deducted on 5th\", ")
            append("\"EMI of Rs X is due on...\", \"scheduled for\", \"upcoming payment\", autopay/mandate notices " +
                "before the actual charge. isReminder: false for completed transactions using past-tense verbs " +
                "like \"debited\", \"credited\", \"spent\", \"has been charged\". A reminder is NOT a completed " +
                "transaction — money hasn't moved yet, so it must never get a bodyRegex (set bodyRegex to null), " +
                "even though isFinancial may still be true for it.\n\n")
            append("When isFinancial is true AND isReminder is false, ALSO generate a regular expression matching ")
            append("this SMS format. It MUST extract these named capture groups:\n")
